...
v_tol_33 = 0.0200	# unit Volt
v_tol_25 = 0.0100	# unit Volt
v_tol_18 = 0.0100	# unit Volt
v_tol_15 = 0.0100	# unit Volt
...
for rpd_0 in [rpd_x    for rpd_x in mts   if rpd_x not in mts_na]:
	...
	if ( abs(vout-3.3)<v_tol_33 ):
		print ( f" V33 : (rpd_0 ) = ( {rpd_0:8.2f} )   Vout={vout:7.4f}   Err={abs(vout-3.3):7.4f} ")
	
	# check V25
	if ( abs(vout-2.5)<v_tol_25 ):
		print ( f" V25 : (rpd_0  ) = ( {rpd_0:8.2f} )   Vout={vout:7.4f}   Err={abs(vout-2.5):7.4f} ")
	
	# check V18
	if ( abs(vout-1.8)<v_tol_18 ):
		print ( f" V18 : (rpd_0 ) = ( {rpd_0:8.2f} )   Vout={vout:7.4f}   Err={abs(vout-1.8):7.4f} ")
	
	# check V15
	if ( abs(vout-1.5)<v_tol_15 ):
		print ( f" V15 : (rpd_0 ) = ( {rpd_0:8.2f} )   Vout={vout:7.4f}   Err={abs(vout-1.5):7.4f} ")
...
for n in range (4):
	...
	if   (n==0):
		(v_target, str_v,rpd_0_vout, rpd_1_vout) = (3.3, "3.3", r_pd33_0, r_pd33_1 )
	elif (n==1):
		(v_target, str_v,rpd_0_vout, rpd_1_vout) = (2.5, "2.5", r_pd25_0, r_pd25_1 )
	elif (n==2):
		(v_target, str_v,rpd_0_vout, rpd_1_vout) = (1.8, "1.8", r_pd18_0, r_pd18_1 )
	elif (n==3):
		(v_target, str_v,rpd_0_vout, rpd_1_vout) = (1.5, "1.5", r_pd15_0, r_pd15_1 )

	for (rpd_0,rpd_1) in zip(rpd_0_vout, rpd_1_vout):
		...
		# check VOUT
		print ( f" {str_v} : (rpd_0, rpd_1) = ( {rpd_0:9.2f}, {rpd_1:9.2f}) ({rpd_0_norm:5.2f}, {rpd_1_norm:5.2f})   Vout={vout:7.4f}   Err={abs(vout-v_target):7.4f}  {rpd_0_e} {rpd_1_e}")
